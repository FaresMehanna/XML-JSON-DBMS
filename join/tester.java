...
public class tester{
		...
		switch(parser.getQueryType()){
		    ...
		    case 13:
		    	if(this.curr_db == null)
		    		...
		    		this.controller.AlterDrop(this.parsedQuery.get(0), this.curr_db, this.parsedQuery.get(1) , this.getFileInstance());
    ...
    private void update(List<String> s) throws SQLException{
	   ...
	   if(this.parser.isWhere())
	   {
  		   ...
  		   if(this.Display == null)
   		   ...
   		   w.clear();
   			for(int i = 0; i < this.Display.size(); i++){
   				...
   				this.output = this.controller.ModifyRow(s.get(0),curr_db, colNames,w ,old, colNames, colValues , this.getFileInstance());
   				w.clear();old.clear();
   			}
		   ...
		   this.output = this.controller.ModifyRow(s.get(0),curr_db, null, null,null, colNames, colValues , this.getFileInstance());
   }
    ...
    private void select(List<String> s) throws SQLException{
    	...
    	if(!s.get(0).equals("*")){
        	...
        	boolean flag = true;
        	this.map.clear();this.mapWithDataTypes.clear();
    		for(int j = 0; j < cols.size(); j++){
    			this.map.put(cols.get(j), Integer.valueOf(j+1));
    			this.mapWithDataTypes.put(cols.get(j), this.controller.getType(this.curr_db, s.get(1), cols.get(j) , this.getFileInstance()));
    			if(this.Display == null)
    				return;
    		...
    		this.map.clear();int cnt = 0;this.mapWithDataTypes.clear();
		    if(!this.Display.isEmpty()){
    			for(String ss : this.Display.get(0).keySet()){
    				this.map.put(ss, Integer.valueOf(cnt++));
    				this.mapWithDataTypes.put(ss, this.controller.getType(this.curr_db, s.get(1), ss , this.getFileInstance()));
    			}
		    }
    	}
